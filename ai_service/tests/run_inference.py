...
def run_on_video(video_path: Path, model_path: Path, out_base: Path, backbone: str = 'mobilenet_v2', confidence_threshold: float = 0.5, save_frames: bool = True):
    ...
    # Save stats to file
    with open("inference_stats.txt", "w") as f:
        f.write(f"Avg latency: {sum(latencies)/len(latencies):.2f}ms\n")
        f.write(f"Min latency: {min(latencies):.2f}ms\n")
        f.write(f"Max latency: {max(latencies):.2f}ms\n")
        f.write(f"Inference FPS: {1000/np.mean(latencies):.2f} fps\n")
        f.write(f"Total inference time: {sum(latencies)/1000:.2f}s\n")
