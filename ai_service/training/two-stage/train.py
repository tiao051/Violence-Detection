...
class Trainer:
    """Trainer class for GTE model."""
    def __init__(self, config: TrainConfig, resume_from: str = None):
        self.config = config
        ...
        self.scheduler = OneCycleLR(
            ...
            div_factor=config.scheduler_max_lr / config.scheduler_min_lr,
            final_div_factor=config.scheduler_min_lr / 1e-9
        )
    ...
    def _measure_total_system(self, gte_model, backbone_constructor, backbone_name, temporal_dim, channels, device):
        """Measures the total FLOPs and parameters of the combined STE+GTE system."""
        self.logger.info("Measuring total system FLOPs and parameters...")
        
        # Create a dummy input for STE
        # STE expects (batch_size, 3, H, W) for a single frame
        # We need to simulate the feature extraction process for a video
        
        # Dummy input for STE backbone (single frame)
        dummy_ste_input = torch.randn(1, 3, 224, 224).to(device) # Assuming common input size
        
        # Instantiate the backbone
        backbone = backbone_constructor(pretrained=False)
        backbone.eval()
        backbone.to(device)
        
        # Get the feature extractor part of the backbone
        if 'mobilenet' in backbone_name or 'efficientnet' in backbone_name:
            # For MobileNet/EfficientNet, features are typically extracted from the 'features' attribute
            ste_feature_extractor = backbone.features
        elif 'mnasnet' in backbone_name:
            # For MNASNet, it's also typically 'layers' or similar
            ste_feature_extractor = backbone.layers
        else:
            raise ValueError(f"Unsupported backbone for FLOPs calculation: {backbone_name}")
            
        # Calculate FLOPs for STE feature extraction (per frame)
        ste_macs, ste_params = profile(ste_feature_extractor, inputs=(dummy_ste_input,), verbose=False)
        
        # Simulate GTE input (output of STE for T/3 frames)
        # (temporal_dim, channels, spatial_size, spatial_size)
        dummy_gte_input = torch.randn(temporal_dim, channels, 7, 7).to(device) # Assuming 7x7 spatial size
        
        # Calculate FLOPs for GTE
        gte_macs, gte_params = profile(gte_model, inputs=(dummy_gte_input,), verbose=False)
        
        # Total FLOPs: (STE FLOPs per frame * temporal_dim) + GTE FLOPs
        # Note: thop reports MACs, which are often ~half of FLOPs.
        # For simplicity, we'll just report MACs as FLOPs.
        total_macs = (ste_macs * temporal_dim) + gte_macs
        total_params = ste_params + gte_params
        
        self.logger.info(f"Total System FLOPs (MACs): {total_macs / 1e9:.2f} G")
        self.logger.info(f"Total System Parameters: {total_params / 1e6:.2f} M\n")
        
        # Clean up dummy models
        del backbone
        del ste_feature_extractor
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.config.epochs):
            ...
            self.logger.info(f"  Val   - Loss: {val_loss:.4f}, Acc: {val_acc:.4f}, Prec: {val_prec:.4f}, Rec: {val_rec:.4f}, F1: {val_f1:.4f}")
            ...
            if val_acc > self.best_val_acc:
                ...
                self._save_model('best_model.pt', epoch=epoch+1)
                self.logger.info(f"Saved best model (val acc: {self.best_val_acc:.4f})")
            
            # Save last model
            self._save_model('last_model.pt', epoch=epoch+1)
    ...
    def _save_model(self, filename: str, epoch: int = 0):
    ...
    def _load_checkpoint(self, checkpoint_path: str):
        ...
        checkpoint_path = Path(__file__).parent / 'checkpoints' / checkpoint_path
        ...
        self.logger.info(f"Resuming from epoch {self.start_epoch} with best validation accuracy {self.best_val_acc:.4f}")
...
def main():
    ...
    parser.add_argument('--dataset', type=str, required=True, choices=['rwf-2000', 'hockey-fight', 'uvd'],
                        help='Dataset to use for training (required: rwf-2000, hockey-fight, or uvd)')
    ...
    parser.add_argument('--resume', type=str, default=None, help='Resume from checkpoint (e.g. best_model.pt)')
    ...
    elif args.dataset == 'uvd':
        extracted_frames_dir = dataset_root / 'UVD' / 'extracted_frames'
    else:
