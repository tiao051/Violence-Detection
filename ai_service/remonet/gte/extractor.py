...
class GTEExtractor(nn.Module):
    ...
    def process(
        ...
        # Class 0: Violence, Class 1: NonViolence (matching DataLoader)
        if probs.dim() == 1:
            # Unbatched case
            violence_prob = float(probs[0].item())
            no_violence_prob = float(probs[1].item()) if self.num_classes == 2 else 1.0 - violence_prob
        else:
            # Batched case - take first sample
            violence_prob = float(probs[0, 0].item())
            no_violence_prob = float(probs[0, 1].item()) if self.num_classes == 2 else 1.0 - violence_prob
