"""
Demo Script - Violence Insights Models

Run this script to see the full output of all analytics models.
This demonstrates how the models analyze violence detection data.

Usage:
    python -m ai_service.insights.demo
    
    or from the ai_service folder:
    python insights/demo.py
"""
...
def demo_mock_data_generation():
    ...
    # Generate 300 events over the last 60 days
...
def demo_time_pattern_analysis(events):
    ...
    print(f"\n⚠️  MOST DANGEROUS: {dangerous['period']} with {dangerous['count']} events ({dangerous['percentage']}%)")
    ...
    # 8. Top Hotspots (Hour + Day combinations)
...
def demo_location_analysis(events):
    ...
    for cam in summary['hotspot_cameras']:
        ...
        bar = "█" * bar_len
        print(f"   {cam['camera_name']:<15} | {bar:<30} {cam['event_count']} events ({cam['percentage']}%)")
    ...
    for i, cam in enumerate(summary['risk_ranking'], 1):
        ...
        print(f"   {icon} {i:<3} {cam['camera_name']:<15} {cam['risk_score']:.3f}   {cam['event_count']:<8} {cam['high_severity_pct']:.1f}%")
...
def main():
    ...
    print("\n")
    print("╔" + "═" * 68 + "╗")
    print("║" + " " * 15 + "🔍 VIOLENCE INSIGHTS DEMO" + " " * 28 + "║")
    print("║" + " " * 12 + "Analytics Models Demonstration" + " " * 25 + "║")
    print("╚" + "═" * 68 + "╝")
