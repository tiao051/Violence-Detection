...
async def websocket_camera_endpoint(websocket: WebSocket, camera_id: str):
    """
    WebSocket endpoint for specific camera threat streaming.

    Args:
        camera_id: Camera ID (e.g., "cam1", "cam2", "cam3", "cam4")

    Clients connect and receive threat detection updates only for that camera.
    """
    await manager.connect(websocket)
    logger.info(f"Camera {camera_id} WebSocket connected")

    try:
        # Get redis_producer from app state
        redis_producer = websocket.app.state.redis_producer
        
        if not redis_producer:
            logger.error(f"Redis producer not available for camera {camera_id}")
            return

        frame_counter = 0

        while True:
            # Continuously stream data for this camera
            try:
                # Get threat for specific camera
                threat_data = await redis_producer.get_latest_threat(camera_id)
                ...
                if threat_data:
                    ...
                    metadata = {
                        ...
                        "violence": threat_data.get("violence", False),
                ...
                logger.error(f"Error streaming camera {camera_id} data: {e}")
                break
